...
import { setTabelogMailConfig } from "actions/tabelogMailConfig";
import { Col, Row, Steps } from "antd";
import TabelogConfirmMail from "components/EmailControllers/Tabelog/TabelogConfirmMail";
import TabelogConFirmMailEnd from "components/EmailControllers/Tabelog/TabelogConfirmMailEnd";
import TabelogRegistration from "components/EmailControllers/Tabelog/TabelogRegistration";
...
export default function TabelogMailConfiguration() {
  ...
  const callAPICreateSuccess = useSelector(
    (state) => state.tabelogConfirmMailReducer.flagCreateSuccess
  );
  const [current, setCurrent] = useState(0);
  const handleClick = async () => {
    if (checkEnd1 && checkEnd2) {
      await dispatch(
        setTabelogMailConfig({
